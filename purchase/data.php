...
if (isset($_GET['i'])) {
 $barcode = $_GET['i'];
  ...
  $check_item="select * from purchase WHERE item='$p_item_name' AND order_id='$invoice' AND done_date=CURDATE()";
    $run_query=mysqli_query($conn,$check_item);

    if(mysqli_num_rows($run_query)>0)
    {
  $results = mysqli_query($conn,"SELECT * FROM purchase WHERE  item='$p_item_name' AND order_id='$invoice' AND done_date=CURDATE()");
     $row = mysqli_fetch_array($results);
     $QTUY=$row['qty'];
     $qttal=$QTUY+$p_quantity;
     $pu_total=$qttal * $p_cost;
     $sql = "UPDATE purchase SET qty='$qttal',total='$pu_total' WHERE  item='$p_item_name' AND order_id='$invoice' AND done_date=CURDATE()";
